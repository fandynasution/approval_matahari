...
class CmDoneController extends Controller
{
    public function Mail(Request $request)
    {
        ...
        try {
            ...
            if (!empty($emailAddresses)) {
                ...
                $cacheFilePath = storage_path('app/mail_cache/send_cmdone/' . date('Ymd') . '/' . $cacheFile);
			...
			->send(new SendCmDoneMail($encryptedData, $dataArray, 'IFCA SOFTWARE - '.$entity_name));
                    ...
                    Log::channel('sendmailapproval')->info('Email CM Done doc_no '.$doc_no.' Entity ' . $entity_cd.' already sent to: ' . $email);
    ...
    public function processData($status='', $encrypt='')
    {
        ...
        if ($exists) {
            $msg1 = [
                "Pesan" => 'You have already made a request to Contract Complete No. ' . $data["doc_no"],
                "St" => 'OK',
                "notif" => 'Restricted !',
        ...
        if (!$exists2) {
            $msg1 = [
                "Pesan" => 'There is no Contract Complete with No. ' . $data["doc_no"],
                "St" => 'OK',
                "notif" => 'Restricted !',
                "image" => "double_approve.png"
            ];
        ...
        return view('email/cmdone/passcheckwithremark', $dataView);
    }
