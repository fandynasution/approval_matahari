...
class CmEntryController extends Controller
{
    public function Mail(Request $request)
    {
        ...
        try {
			...
			->send(new SendCmEntryMail($encryptedData, $dataArray, 'IFCA SOFTWARE - '.$entity_name));
                    ...
                    Log::channel('sendmailapproval')->info('Email CM Entry doc_no '.$doc_no.' Entity ' . $entity_cd.' already sent to: ' . $email);
    ...
    public function processData($status='', $encrypt='')
    {
        ...
        if ($exists) {
            $msg1 = [
                "Pesan" => 'You Have Already Made a Request to Contract Entry No. ' . $data["doc_no"],
                "St" => 'OK',
                "notif" => 'Restricted !',
        ...
        if (!$exists2) {
            $msg1 = [
                "Pesan" => 'There is no Contract Entry with No. ' . $data["doc_no"],
                "St" => 'OK',
                "notif" => 'Restricted !',
                "image" => "double_approve.png"
            ];
        ...
        return view('email/cmentry/passcheckwithremark', $dataView);
    }
